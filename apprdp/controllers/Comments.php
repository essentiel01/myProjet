 ...
 */
	public function postComments()
	{
		...
		foreach ($comments as $row) {
			$output .= '<div><div> Par ' . $row->userFirstName . ' ' . $row->userLastName . ' le ' . $row->commentDate . '</div> <div>' . $row->commentContent . '</div><button type="button" class="reply btn btn-lg btn-primary" data-commentId=' . $row->commentId . '>Répondre</button></div>';
			if (isset($row->children)) {
				foreach ($row->children as $row) {
					$output .= '<div style="margin-left: 40px;"><div> Par ' . $row->userFirstName . ' ' . $row->userLastName . ' le ' . $row->commentDate . '</div> <div>' . $row->commentContent . '</div></div>';
				}
			}
		}
		echo $output;
